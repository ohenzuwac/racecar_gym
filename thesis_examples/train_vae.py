...
def train():
    ...
    for i in range(num_csvs):
        df = pd.read_csv(f"/home/jupyter/racecar_gym/thesis_examples/csv_files/0501/Episode{i}.csv")
        df = df.head(4000)
        
        #subsampling, every 8th row for 500 total points along the full trajectory
        df = df.iloc[::8,:]
    ...
    for epoch in range(epochs):
        ...
        if epoch % chkpoint == 0 or epoch == 999:
            torch.save(model,"/home/jupyter/VAEmodels/0501_models_1csv/model{}.pt".format(epoch))
...
def get_test():
    ...
    for i in range(100,200):
        df = pd.read_csv(f"/home/jupyter/racecar_gym/thesis_examples/csv_files/0501_testset/Episode{i}.csv")
        df = df.head(4000)
        df = df.iloc[::8,:]
...
def plot_traj_singles(x,model,epoch,slctr):
    ...
    plt.ylabel("Y Position")
    plt.title(titles[slctr])
    plt.legend(["Agent A", "Agent B", "Agent C", "Agent D","Agent E","Agent F"], bbox_to_anchor=(1.04, 1), loc = "upper left")
    plt.show()
...
def latent_space_plotter(z_values_x,z_values_y,z_values):
    #fig = plt.figure()
    ...
    z_values_x = np.array(z_values_x)
    z_values_y = np.array(z_values_y)
    
    labels = [0,1,2,3,4,5]*100
    labels = np.array(labels)
    colors = {0:"darkgreen", 1:"lightgreen", 2:"yellow", 3:"gold",4:"tomato",5:"darkred"}
    ...
    for lbl in np.unique(labels):
        ...
        plt.scatter(z_values_x[idx],z_values_y[idx], c = colors[lbl], label = lbl)
   
    #plt.scatter(z_values_x, z_values_y, c = labels.astype(float))
    plt.legend(["Agent A", "Agent B", "Agent C", "Agent D","Agent E","Agent F"])
    plt.savefig('example_latentspace.png')
...
if __name__ == "__main__":
    #model, _ = train()
    model = torch.load("/home/jupyter/VAEmodels/0501_models/model999.pt")
    #sample = model.sample(1,torch.device('cpu'))
    ...
    for x in test_loader:
        test_traj.append(x)
        if len(test_traj) == 100:
            break
    
    #print("test trajectoris /n", test_traj[0])
    plot_traj_singles(test_traj[13:19],model,501,2)
    
    for agent in test_traj[13:19]:
        VAE_gen.append(model.generate(agent[...,:2]))
    
    
    plot_traj_singles(VAE_gen,model,501,1)
    ...
    latent_space_plotter(z_values_x,z_values_y,z_values)
